...
从数组尾部开始查找，满足提供的测试函数的一个元素的值，否则返回undefined  
Array.prototpye.findLastIndex()  
从数组尾部开始查找，满足提供的测试函数的一个元素的索引，若没有找到对应元素则返回-1 
...
Array.isArray()  
返回true或false,用于确定传递的值是否是一个Array  
...
返回函数调用值，对数组中的每个元素按序执行一个提供的函数，每次运行此函数将会将之前的计算结果欧威参数传入，最后将其结果汇总为单个返回值  
