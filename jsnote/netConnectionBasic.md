...
客户端发出的消息称为 HTTP 请求，服务端返回的消息称为 HTTP 响应
...
恶意 js 脚本上传攻击服务器，或者恶意钓鱼网站进行脚本视觉欺骗等

如何解决：

1. 不信任客户端操作，业务主流程关键节点校验，用户输入及操作 API 校验
...
1. 头部字段压缩
1. 多路复用，同一 TCP 连接可并行传输多个 HTTP 请求和响应，请求数据无序，浏览器组装
1. 服务端主动推送缓存

http3

1. 优化头部压缩
2. 放弃 tcp,通过 QUIC(udp)建立，解决了队头堵塞问题等问题
