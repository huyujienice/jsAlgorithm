...
### render函数为什么是函数字符串而不是函数本身
1. 跨平台兼容性:不同的js引擎可能对函数对象的处理有所不同      
2. 内存占用：函数对象通常比字符串占用更多内存，因为函数对象包含了执行上下文及闭包等信息      


# vue nextTick
nextTick本质就是执行延迟回调的钩子函数，接受一个callback函数，在下次dom更新循环结束之后执行延迟callback。  
**操作dom节点是同步阻塞操作**     
...
有一个全局对象存储副作用函数队列，一个pending标识，当pending为true的时候，副作用函数放入下一次的队列中，并且返回一个执行的Promise命令     
...
1. 依赖收集：注册副作用函数的时候(生成render渲染函数)使用了响应式对象的响应式属性值，会触发其读取操作,副作用函数插入MVVM架构中的Set队列      
2. 派发更新：当响应式对象的响应式属性值发生改变的时候，会触发其设置操作,刷新副作用函数队列       
...
## CacheHandle
开启CacheHandle事件缓存后，render函数将绑定的事件生成固定的内联函数，由内联参数传递代替上下文对象进行传参，不再为事件生成全新函数，减少内存分配，避免子组件无效更新。          
...
特点：    
1. 比较只会同层进行，不会跨层比较  
2. 根据vdom类型及key值等条件来判断是否可复用vdom     


基础的diff算法就是拿新节点数组中的节点去旧节点数组中查找，通过key和节点类型等来递归判断是否能够复用当前节点。         
...
history模式，通过history.pushState和history.replaceState方法修改url地址      
window.onpopstate只能监听浏览器前进后退改变url，如果是a调整可以拦截a标签的点击事件来监听url变化     
