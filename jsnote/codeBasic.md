...
3. RSA,用于加密和数字签名
4. DSA,用于数字签名,通常与 SHA 结合使用
5. ECC,基于椭圆曲线的加密算法,常用于移动设备和物联网设备
...
3. 转账阶段（必须带有效身份）：
    - 浏览器把 { action: "transfer", amount: 1000, nonce: "..." } 用 AES 加密，并自动附带 JWT。
    - 服务端：
        - 用 RSA 私钥解密 AES 密钥 → 解密 JSON
        - 验证 JWT 签名 → 确认用户身份
        - 检查 nonce 是否已用 → 防重放
        - 核对余额 → 执行转账
...
1. 浏览器预装 CA 机构的数字证书验证程序，服务器给与浏览器数字证书，验证程序能保证数字证书的有效性，从而获取到服务器信息及公钥
    - 验证程序包含公钥私钥，数字证书通过公钥加密，浏览器内验证程序通过数字证书链及私钥解密来获取证书内服务器信息
    - 采用 CA 机构验证服务器公钥的方案能够解决公钥信任问题(网站是否是正常网站，是否备案过等等)及中间人攻击的问题
...
1. AND 运算:都为 1 才为 1，其余为 0
2. OR 运算：只要有一个为 1 就为 1，其余为 0
3. XOR 运算：相同则为 0，不同为 1
