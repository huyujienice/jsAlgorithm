...
对老生代(Major GC) 
1. 标记(Marking):标识哪些对象为活的，可通过 Stack 指针进行递归遍历，可访问的对象为活的  
...
Just-In-Time 编译，通过将字节码编译成机器码，提升 js 代码的执行效率，是运行时优化

1. 收集：识别频繁执行的热点代码并收集
2. 优化：通过内联函数，死区代码消除，循环展开等技术优化代码
3. 生成：将优化后的字节码编译成机器码，并存入缓存
4. 执行：执行引擎执行机器码
