...
function judgeFind(arr, str, i, j, k, arrs = []) {
  ...
  if (r1) return false;
  arrs.push([i, j]);
  judgeFind(arr, str, i - 1, j, k + 1, [...arrs]);
  judgeFind(arr, str, i + 1, j, k + 1, [...arrs]);
  judgeFind(arr, str, i, j - 1, k + 1, [...arrs]);
  judgeFind(arr, str, i, j + 1, k + 1, [...arrs]);
  if (k == str.length - 1) {
    console.log(`found!`);
    results.push(arrs);
